 ...
 * Subject : CJ Mall 
 * Name : TC_135
 * Scenario : 전시 > 마이존 클릭 > 비로그인 상태에서 마이존 클릭 시 로그인 페이지로 이동
 * Assertion : 브라우저 Title "로그인" 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */

public class Mobile_TC135 {
	...
	@Test
	public void MyzoneLogout() throws Exception {
		...
		try {
		    ...
		    // 브라우저 Title "로그인" 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "로그인");
		    if(isExist2 && driver.getTitle().equals("로그인")){
		    	System.out.println("[TC_135] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_135] failure : 브라우저 Title '로그인' 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
