...
public class Mobile_TC010 {
	...
	@Test
	public void productPriceView() throws Exception {
		...
		try {
			...
			// 상품가격 XPath Text 'null' 여부 체크
			isExist3 = existElement(driver, By.xpath(".//*[@id='ct']/div/div[4]/div/a/div[2]/span[2]"), "상품가격"); // 상품가격
			if(isExist3){
				if(!"".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[4]/div/a/div[2]/span[2]")).getText())){
					System.out.println("[TC_010] success");
					assertTrue(true);
					return;
				} else {
					System.out.println("[TC_010] failure : 상품가격 Text null...");
					assertTrue(false);
				}
			} else {
				System.out.println("[TC_010] failure : 생방송 상품 시간 아님. 다른 시간대 검증필요.");
				assertTrue(true);
			}
		} catch (Exception e) {
			e.printStackTrace();
