 ...
 * Subject : CJ Mall 
 * Name : TC_153
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 주문내역/배송현황 - 주문취소/교환/반품  상세내역 페이지로 이동
 * Assertion : "주문취소/교환/반품 " Title 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC153 {
	...
	@Test
	public void MyzoneOrderWithdrawa() throws Exception {
		...
		try {
			...
			// 주문취소/교환/반품 버튼 클릭
	  		...
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("주문취소/교환/반품  타이틀 기다림");
	  		
		    // 브라우저 Title "주문취소/교환/반품  " 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "주문취소/교환/반품");
		    if(isExist2 && driver.getTitle().equals("주문취소/교환/반품")){
		    	System.out.println("[TC_153] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_153] failure : 브라우저 Title '주문취소/교환/반품' 불일치");
				assertTrue(false);
			}
