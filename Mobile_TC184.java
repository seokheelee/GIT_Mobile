 ...
 * Subject : CJ Mall 
 * Name : TC_184
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 방송알리미 > 휴대폰번호  
 * Assertion : 휴대폰번호 Text 이동 확인
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC184 {
	...
	@Test
	public void MyzoneNotificationPhoneNumber() throws Exception {
		...
		try {
			...
			element = driver.findElement(By.xpath(".//*[@id='ct']/div[3]/ul/li[8]/a"));
	  		...
	  		// 휴대폰번호 Text 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div/form/div[1]/dl/dt[1]/label")));
		  	System.out.println("휴대폰번호 Text 기다림");
			
			// 휴대폰번호 Text 체크
			if ("휴대폰번호".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/form/div[1]/dl/dt[1]/label")).getText())) {
				System.out.println("[TC_184] success");
				assertTrue(true);
				return;
				
			} else {
				System.out.println("[TC_184] failure : 휴대폰번호 불일치");
				assertTrue(false);
			}
