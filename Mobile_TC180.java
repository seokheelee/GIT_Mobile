 ...
 * Subject : CJ Mall 
 * Name : TC_180
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑찜 > 전체선택 > 전체해제
 * Assertion : 전체선택 Text 확인
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC180 {
	...
	@Test
	public void MyzoneShoppingzzimAllChose() throws Exception {
		...
		try {
			...
			element = driver.findElement(By.xpath(".//*[@id='checkAllText']/a"));
	  		element.click();
		  	System.out.println("전체해제 버튼 클릭");						
	  			  		
			// 전체선택 text 체크
			if ("전체선택".equals(driver.findElement(By.xpath(".//*[@id='checkAllText']/a")).getText())) {
				System.out.println("[TC_180] success");
				assertTrue(true);
				return;
				
			} else {
		    	System.out.println("[TC_180] failure : 전체선택 Text 불일치");
				assertTrue(false);
			}
