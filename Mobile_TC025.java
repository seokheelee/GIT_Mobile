 ...
 * Subject : CJ Mall 
 * Name : TC_025
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "뷰티/보석" 카테고리 노출
 * Assertion : "뷰티/보석" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC025 {
	...
	@Test
	public void beautyJewelryView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	// "뷰티/보석" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[4]/a/span"),"뷰티/보석");
		  	if(isExist){
		  		if("뷰티/보석".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[4]/a/span")).getText())){
		  			System.out.println("[TC_025] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_025] failure : '뷰티/보석' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_025] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
