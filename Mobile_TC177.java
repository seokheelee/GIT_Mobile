 ...
 * Subject : CJ Mall 
 * Name : TC_177
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑찜 > 삭제
 * Assertion : 삭제 버튼 Text 체크
...
public class Mobile_TC177 {
	...
	@Test
	public void MyzoneShoppingzzimDelete() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='jjimList']/li[1]/div[2]/div[1]/p/a")));
		  	System.out.println("삭제 버튼 기다림");

			// 삭제 text 체크
			if ("삭제".equals(driver.findElement(By.xpath(".//*[@id='jjimList']/li[1]/div[2]/div[1]/p/a")).getText())) {
				System.out.println("[TC_177] success");
				assertTrue(true);
				return;
				
			} else {
		    	System.out.println("[TC_177] failure : 장바구니 불일치");
				assertTrue(false);
			}
