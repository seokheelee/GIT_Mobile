 ...
 * Subject : CJ Mall 
 * Name : TC_157
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 주문취소/교환/반품 > 신청내역조회 > 3개월 탭으로 이동
 * Assertion : "3개월" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC157 {
	...
	@Test
	public void MyzoneeOrderWithdrawa3month() throws Exception {
		...
		try {
			...
			element = driver.findElement(By.xpath(".//*[@id='ct']/div[3]/ul/li[2]/div/p/a"));
	  		...
	  		// 3개월 탭 'V' 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div[3]/ul/li[2]/div/p/span")));			
		  	System.out.println("3개월 탭 'V' 기다림");
		  	
		  	Thread.sleep(3000);
			
			// 3개월 탭 'V' text 체크
			if ("".equals(driver.findElement(By.xpath(".//*[@id='ct']/div[3]/ul/li[2]/div/p/span")).getText())) {
				System.out.println("[TC_157] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_157] failure : '3개월' 불일치");
				assertTrue(false);
			}
