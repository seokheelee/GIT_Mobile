 ...
 * Subject : CJ Mall 
 * Name : TC_057
 * Scenario : 전시 > TV쇼핑 > 5%적립신청 > 버튼 선택시 해당 페이지로 이동
 * Assertion : img alt "CJmall 앱으로 TV방송상품 구매시 5% CJ ONE 포인트" 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC057 {
	...
	@Test
	public void SavingButtonView() throws Exception {
		...
		try {
			...
			if ("5% 적립신청".equals(driver.findElement(By.xpath(".//*[@id='tvLive']/div[5]/ul/li[1]/a")).getText())) {
			  	driver.findElement(By.xpath(".//*[@id='tvLive']/div[5]/ul/li[1]/a")).click(); 
			  	System.out.println("5% 적립신청 클릭");
			} else {
		  	...
		  	// "CJmall 앱으로 TV방송상품 구매시 5% CJ ONE 포인트" 이미지 뜰 때까지 기다림.
		    wait = new WebDriverWait(driver, waitTime);
		  	wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='evt_wrap']/div[1]/h1/img")));
		  	System.out.println("CJmall img alt 기다림");
		  	
		  	// "CJmall 앱으로 TV방송상품 구매시 5% CJONE 포인트" img alt 체크
		  	// CJONE / CJ ONE 계속 바뀌어서 두개 중 하나가 일치하는지 확인
	  		if("CJmall 앱으로 TV방송상품 구매시 5% 적립금 추가지급".equals(driver.findElement(By.xpath(".//*[@id='evt_wrap']/div[1]/h1/img")).getAttribute("alt"))
	  				|| "CJmall 앱으로 TV방송상품 구매시 5% CJ ONE 포인트".equals(driver.findElement(By.xpath(".//*[@id='evt_wrap']/div[1]/h1/img")).getAttribute("alt"))){
	  			System.out.println("[TC_057] success");
				assertTrue(true);
				return;
	  		} else {
	  			System.out.println("[TC_057] failure : 'CJmall 앱으로 TV방송상품 구매시 5% 적립금 추가지급' img alt 불일치");
	  			assertTrue(false);
	  		   	return;
	  		}
		} catch (Exception e) {
