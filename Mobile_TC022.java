 ...
 * Subject : CJ Mall 
 * Name : TC_022
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "카테고리 전체" 카테고리 노출
 * Assertion : "카테고리 전체" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC022 {
	...
	@Test
	public void categoryAllView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	// "카테고리 전체" Text 체크.//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[1]/a/span
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[1]/a/span"),"카테고리 전체");
		  	if(isExist){
		  		if("카테고리 전체".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[1]/a/span")).getText())){
		  			System.out.println("[TC_022] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_022] failure : '카테고리 전체' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_022] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
