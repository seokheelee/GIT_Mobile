 ...
 * Subject : CJ Mall 
 * Name : TC_063
 * Scenario : 전시 > 하단 '카테고리' 버튼
 * Assertion :  "고객센터" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC063 {
	...
	public void HomeButton() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[2]/a")));
		  	System.out.println("하단 '카테고리' 버튼 기다림");
			
			 // 하단 '카테고리' 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[2]/a")).click();
		  	System.out.println("하단 '카테고리' 버튼 클릭");
			
			// 고객센터 Text 기다림.
		    wait = new WebDriverWait(driver, waitTime);
		  	wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='view']/div/div/div[4]/a")));
		  	System.out.println("고객센터 Text 기다림");
		  	
		  	// 고객센터 Text 체크
			if ("고객센터".equals(driver.findElement(By.xpath(".//*[@id='view']/div/div/div[4]/a")).getText())) {
				System.out.println("[TC_063] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_063] failure : '고객센터' Text 불일치");
				assertTrue(false);
			}
