 ...
 * Subject : CJ Mall 
 * Name : TC_143
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑플러스 클릭시 내 쿠폰 리스트 - 쇼핑플러스 상세내역 페이지로 이동
 * Assertion : "내 쿠폰 리스트 - 쇼핑플러스" Title 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC143 {
	...
	@Test
	public void MyzoneShoppingPlus() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
	  		...
	  		// 쇼핑플러스 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='stPlus']")));
		  	System.out.println("쇼핑플러스 기다림");

			// 쇼핑플러스 클릭
			element = driver.findElement(By.xpath(".//*[@id='stPlus']"));
	  		element.click();
		  	System.out.println("쇼핑플러스 클릭");

	  		// 내 쿠폰 리스트 - 쇼핑플러스 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("내 쿠폰 리스트 - 쇼핑플러스 기다림");
	  		
		    // 브라우저 Title "내 쿠폰 리스트 - 쇼핑플러스" 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "내 쿠폰 리스트 - 쇼핑플러스");
		    if(isExist2 && driver.getTitle().equals("내 쿠폰 리스트 - 쇼핑플러스")){
		    	System.out.println("[TC_143] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_143] failure : 브라우저 Title '내 쿠폰 리스트 - 쇼핑플러스' 불일치");
				assertTrue(false);
			}
