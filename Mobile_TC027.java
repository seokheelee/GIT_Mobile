 ...
 * Subject : CJ Mall 
 * Name : TC_027
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "유아동/도서" 카테고리 노출
 * Assertion : "유아동/도서" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC027 {
	...
	@Test
	public void kidsBookView() throws Exception {
		...
		try {
			...
			// 뷰티/보석 클릭 (카테고리바 이동 목적)
		  	...
		  	// "유아동/도서" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[6]/a/span"),"유아동/도서");
		  	if(isExist){
		  		if("유아동/도서".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[6]/a/span")).getText())){
		  			System.out.println("[TC_027] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_027] failure : '유아동/도서' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_027] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
