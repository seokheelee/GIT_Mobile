 ...
 * Subject : CJ Mall 
 * Name : TC_064
 * Scenario : 전시 > 하단 '검색' 버튼
 * Assertion :  "인기검색어" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC064 {
	...
	public void HomeButton() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[3]/a")));
		  	System.out.println("하단 '검색' 버튼 기다림");
			
			 // 하단 '검색' 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[3]/a")).click();
		  	System.out.println("하단 '검색' 버튼 클릭");
			
			// "인기검색어" Text 기다림.
		    wait = new WebDriverWait(driver, waitTime);
		  	wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='main_wrap']/div[2]/ul/li[1]/a")));
		  	System.out.println("'인기검색어' text 기다림");
		  	
		  	// "인기검색어" Text 체크
			if ("인기검색어".equals(driver.findElement(By.xpath(".//*[@id='main_wrap']/div[2]/ul/li[1]/a")).getText())) {
				System.out.println("[TC_064] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_064] failure : '인기검색어' Text 불일치");
				assertTrue(false);
			}
