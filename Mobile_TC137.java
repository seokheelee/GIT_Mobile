 ...
 * Subject : CJ Mall 
 * Name : TC_137
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > CJ ONE 포인트 클릭시  CJ ONE 포인트 상세내역 페이지로 이동
 * Assertion : 브라우저 Title "CJ ONE 포인트 상세내역" 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */

public class Mobile_TC137 {
	...
	@Test
	public void MyzoneCJONEPoint() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
	  		...
	  		// CJ ONE 포인트 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div[1]/ul[1]/li[2]/span[2]/a/strong")));
		  	System.out.println("CJ ONE 포인트 기다림");

			// CJ ONE 포인트 클릭
			element = driver.findElement(By.xpath(".//*[@id='ct']/div[1]/ul[1]/li[2]/span[2]/a/strong"));
	  		element.click();
		  	System.out.println("CJ ONE 포인트 클릭");

	  		// CJ ONE 포인트 상세내역 text 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("CJ ONE 포인트 상세내역 text 기다림");
	  		
		    // 브라우저 Title "CJ ONE 포인트 상세내역" 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "CJONE 포인트 상세내역");
		    if(isExist2 && driver.getTitle().equals("CJONE 포인트 상세내역")){
		    	System.out.println("[TC_137] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_137] failure : 브라우저 Title 'CJONE 포인트 상세내역' 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
