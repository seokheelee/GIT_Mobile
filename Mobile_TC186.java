 ...
 * Subject : CJ Mall 
 * Name : TC_186
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 방송알리미 > Email  
 * Assertion : Email Text 확인
...
public class Mobile_TC186 {
	...
	@Test
	public void MyzoneNotificationEmail() throws Exception {
		...
		try {
			...
			// 방송알리미 버튼 클릭
	  		...
	  		// 이메일설정 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div/form/div[2]/dl/dt[1]/label")));
		  	System.out.println("이메일설정 기다림");
			
			// 이메일설정 Text 체크
			if ("이메일설정".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/form/div[2]/dl/dt[1]/label")).getText())) {
				System.out.println("[TC_186] success");
				assertTrue(true);
				return;
				
			} else {
				System.out.println("[TC_186] failure : 이메일설정 불일치");
				assertTrue(false);
			}
