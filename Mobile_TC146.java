 ...
 * Subject : CJ Mall 
 * Name : TC_146
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 주문내역/배송현황 - 현재일부터 3개월 검색 페이지로 이동
 * Assertion : "3개월" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC146 {
	...
	@Test
	public void Myzone3month() throws Exception {
		...
		try {
			...
			// 3개월 탭 클릭
			element = driver.findElement(By.xpath(".//*[@id='ct']/div[2]/ul/li[2]/div/p/a"));
	  		element.click();
		  	System.out.println("3개월 탭 클릭");
	  		
	  		// 3개월 탭 'V' 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div[2]/ul/li[2]/div/p/span")));			
		  	System.out.println("3개월 탭 'V' 기다림");			
		  	
		  	Thread.sleep(3000);
			
			// 3개월 탭 'V' text 체크
			if ("".equals(driver.findElement(By.xpath(".//*[@id='ct']/div[2]/ul/li[2]/div/p/span")).getText())) {
				System.out.println("[TC_146] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_146] failure : '3개월' 불일치");
				assertTrue(false);
			}
