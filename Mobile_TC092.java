 ...
 * Subject : CJ Mall 
 * Name : TC_092
 * Scenario : 전시 > 상단메뉴 > 로그인화면 > 아이디 입력 > 자동로그인 버튼 클릭 > 로그인 > Olive Young 화면 전환 > CJmall 화면 전환 > 상단메뉴
 * Assertion : ~님 Text 체크
...
public class Mobile_TC092 {
	...
	@Test
	public void AutoLoginCheck() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/dl/dt/h1/a")));
		  	System.out.println("CJMall Title 기다림");

			// 올리브영 화면전환
			driver.get("http://mw.oliveyoungshop.com/m/main.jsp");
		  	System.out.println("올리브영 화면전환");
						
	  		// Olive Young Title 기다림
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='header']/div/h1/a")));
		  	System.out.println("Olive Young Title 기다림");
			
			// CJmall 화면전환
			driver.get(url);
		  	System.out.println("CJmall 화면전환");
			...
			driver.findElement(By.xpath(".//*[@id='o1h_header_wrap']/dl/dt/a")).click();
	  		...
	  		// ~님 기다림
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='wrapper_cate_left']/dl/dt/span")));
		  	System.out.println("~님 기다림");
			
		  	// "~님" Text 체크
			if(driver.findElement(By.xpath(".//*[@id='wrapper_cate_left']/dl/dt/span")).getText().indexOf("님") >=0){
				System.out.println("[TC_092] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_092] failure : '~님' Text 불일치");
				assertTrue(false);
			}
