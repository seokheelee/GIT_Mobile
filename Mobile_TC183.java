 ...
 * Subject : CJ Mall 
 * Name : TC_183
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 방송알리미
 * Assertion : 방송알리미 Title 이동 확인
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC183 {
	...
	@Test
	public void MyzoneNotification() throws Exception {
		...
		try {
			...
			// 방송알리미 버튼 클릭
	  		...
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("방송알리미 Title 기다림");
	  				
			// 방송알리미 Title 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "방송알리미");
		    if(isExist2 && driver.getTitle().equals("방송알리미")){
		    	System.out.println("[TC_183] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_183] failure : 브라우저 Title '방송알리미' 불일치");
				assertTrue(false);
			}
