 ...
 * Subject : CJ Mall 
 * Name : TC_144
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 주문내역/배송현황 - 주문내역/배송현황 상세내역 페이지로 이동
 * Assertion : "주문내역/배송현황" Title 체크
...
public class Mobile_TC144 {
	...
	@Test
	public void MyzoneOderAndDelivery() throws Exception {
		...
		try {
			...
			// 주문내역/배송현황  버튼 클릭
	  		...
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("주문내역/배송현황 타이틀 기다림");
	  		
		    // 브라우저 Title "주문내역/배송현황 " 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "주문내역/배송현황");
		    if(isExist2 && driver.getTitle().equals("주문내역/배송현황")){
		    	System.out.println("[TC_144] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_144] failure : 브라우저 Title '주문내역/배송현황' 불일치");
				assertTrue(false);
			}
