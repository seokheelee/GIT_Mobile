 ...
 * Subject : CJ Mall 
 * Name : TC_024
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "잡화/스포츠카테고리 노출
 * Assertion : "잡화/스포츠" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC024 {
	...
	@Test
	public void goodsSportsView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	// "잡화/스포츠" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[3]/a/span"),"잡화/스포츠");
		  	if(isExist){
		  		if("잡화/스포츠".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[3]/a/span")).getText())){
		  			System.out.println("[TC_024] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_024] failure : '잡화/스포츠' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_024] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
