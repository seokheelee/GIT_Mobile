 ...
 * Subject : CJ Mall 
 * Name : TC_053
 * Scenario : 전시 > TV쇼핑 > 지금 방송중인 다른상품 > 같은 시간대 방송상품 "지금 방송중인 다른상품" 문구 노출
...
//				((JavascriptExecutor)driver).executeScript("arguments[0].scrollIntoView();", driver.findElement(By.xpath(".//*[@id='tvLive']/div[3]/div/a/div[1]/img")));
		  	...
		  	if(isExist)
		  	{
		  		if(driver.findElement(By.xpath(".//*[@id='tvLive']/div[4]/h4")).getText().contains("지금 방송중인 다른상품")){
		  			System.out.println("[TC_053] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_053] failure : '지금 방송중인 다른상품' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_053] Success : '지금 방송중인 다른상품' 방송시간대 아님. 여기서 종료 ");
	  			assertTrue(true);
	  		   	return;
		  	}
			} catch (Exception e) {
