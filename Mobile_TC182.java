 ...
 * Subject : CJ Mall 
 * Name : TC_182
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑찜 > 계속쇼핑
 * Assertion : 마이존 Title 이동 확인
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC182 {
	...
	@Test
	public void MyzoneShoppingzzimShopping() throws Exception {
		...
		try {
			...
			if ("마이존".equals(driver.findElement(By.xpath(".//*[@id='o1h_header_wrap']/div/span")).getText())) {
				System.out.println("[TC_182] success");
				assertTrue(true);
				return;
				
			} else {
		    	System.out.println("[TC_182] failure : 마이존 Title 불일치");
				assertTrue(false);
			}
