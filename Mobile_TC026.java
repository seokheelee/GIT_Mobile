 ...
 * Subject : CJ Mall 
 * Name : TC_026
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "식품/주방" 카테고리 노출
 * Assertion : "식품/주방" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC026 {
	...
	@Test
	public void foodKitchenView() throws Exception {
		...
		try {
			...
			// 뷰티/보석 클릭 (카테고리바 이동 목적)
		  	...
		  	// "식품/주방" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[5]/a/span"),"식품/주방");
		  	if(isExist){
		  		if("식품/주방".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[5]/a/span")).getText())){
		  			System.out.println("[TC_026] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_026] failure : '식품/주방' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_026] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
