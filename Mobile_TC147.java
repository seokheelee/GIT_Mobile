 ...
 * Subject : CJ Mall 
 * Name : TC_147
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 주문내역/배송현황 - 현재일부터 6개월 검색 페이지로 이동
 * Assertion : "6개월" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC147 {
	...
	@Test
	public void Myzone6month() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div[2]/ul/li[3]/div/p/a")));
		  	System.out.println("6개월 탭  기다림");
	  		
			// 6개월 탭 클릭
			element = driver.findElement(By.xpath(".//*[@id='ct']/div[2]/ul/li[3]/div/p/a"));
	  		element.click();
		  	System.out.println("6개월 탭 클릭");
	  		
	  		// 6개월 탭 'V' 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div[2]/ul/li[3]/div/p/span")));			
		  	System.out.println("6개월 탭 'V' 기다림");			
		  	
		  	Thread.sleep(3000);
			
			// 6개월 'V' text 체크
			if ("".equals(driver.findElement(By.xpath(".//*[@id='ct']/div[2]/ul/li[3]/div/p/span")).getText())) {
				System.out.println("[TC_147] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_147] failure : '6개월' 불일치");
				assertTrue(false);
			}
