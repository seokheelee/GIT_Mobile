 ...
 * Subject : CJ Mall 
 * Name : TC_013
 * Scenario : 전시 > 오클락 딜 > 지금 방송중인 다른상품 > 같은 시간대 방송상품 "지금 방송중인 다른상품" 문구 노출
 * Assertion : "지금 방송중인 다른상품" Text 체크 (보험/상담 상품 시간이 아닌 경우만 가능, 지금 방송중인 다른상품이 있는 경우만 가능)
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC013 {
	...
	@Test
	public void otherProductView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	// "지금 방송중인 다른상품" Text 체크 (보험/상담 상품 시간이 아닌 경우만 가능, 지금 방송중인 다른상품이 있는 경우만 가능)
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[5]/h4"),"지금 방송중인 다른상품");
		  	if(isExist)
		  	{
		  		if(driver.findElement(By.xpath(".//*[@id='ct']/div/div[5]/h4")).getText().contains("지금 방송중인 다른상품")){
		  			System.out.println("[TC_013] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_013] failure : '지금 방송중인 다른상품' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_013] failure : '지금 방송중인 다른상품' 방송시간대 아님. 여기서 종료 ");
	  			assertTrue(true);
	  		   	return;
		  	}
		} catch (Exception e) {
