 ...
 * Subject : CJ Mall 
 * Name : TC_187
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 방송알리미 > Email > 수신동의  
 * Assertion : 수신동의 '아니오' '예' 체크 후 수신동의 Text 확인
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC187 {
	...
	@Test
	public void MyzoneNotificationEmailReciveAsscent() throws Exception {
		...
		try {
			...
			// 스크롤 다운 (마이존 하단 내역 탐색)
	  		...
	  		// 수신동의 '아니오' 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='emaDd']/span[2]/label")));
		  	System.out.println("수신동의 '아니오' 기다림");
			
			// 수신동의 '아니오' 버튼 클릭
			element = driver.findElement(By.xpath(".//*[@id='emaDd']/span[2]/label"));
			element.click();
		  	System.out.println("수신동의 '아니오' 버튼 클릭");
			
	  		// 수신동의 '예' 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='emaDd']/span[1]/label")));
		  	System.out.println("수신동의 '예' 기다림");
			
			// 수신동의 '예'버튼 클릭
			element = driver.findElement(By.xpath(".//*[@id='emaDd']/span[1]/label"));
			element.click();			
		  	System.out.println("수신동의 '예'버튼 클릭");
			
			// 이메일설정 Text 체크
			if ("수신동의".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/form/div[2]/dl/dt[2]")).getText())) {
				System.out.println("[TC_187] success");
				assertTrue(true);
				return;
				
			} else {
				System.out.println("[TC_187] failure : 수신동의 불일치");
				assertTrue(false);
			}
			...
			System.out.println("=====TestCase end======");
			 ...
			 java.lang.Process process = runtime.exec("cmd /c C:\\adbserver.bat");
			 process.waitFor();
		}
	}
