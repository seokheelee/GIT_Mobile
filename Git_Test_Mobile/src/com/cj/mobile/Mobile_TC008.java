 ...
 * Subject : CJ Mall 
 * Name : TC_008
 * Scenario : 전시 > 오클락 딜 > 방송상품 > 남은시간 노출 
 * Assertion : '00:00:00 남음' Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */

public class Mobile_TC008 {
	...
	@Test
	public void countdownView() throws Exception {
		...
		try {
			...
			// 생방송 ONAIR 객체 체크
			boolean isExist3 = existElement(driver, By.xpath(".//*[@id='ct']/div/div[4]/div/a"), ""); // ONAIR
			if(isExist3){
			...
			// 생방송 ONAIR 00:00:00 남음 체크 
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='ct']/div/div[4]/div/div[1]/span[1]"), "ONAIR"); // ONAIR
			if(isExist2){
				// 00:00:00 남음 뜰 때까지 기다림.
				wait = new WebDriverWait(driver, waitTime);
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='ct']/div/div[4]/div/div[1]/span[2]")));
				
				// 해당 Text 에 '남음' 포함하는지 확인
				if(driver.findElement(By.xpath(".//*[@id='ct']/div/div[4]/div/div[1]/span[2]")).getText().indexOf("남음") >=0){
					System.out.println("[TC_008] success");
					assertTrue(true);
					return;
				} else {
					System.out.println("[TC_008] failure : '00:00:00 남음' Text 불일치");
					assertTrue(false);
				}
			} else {
				System.out.println("[TC_008] failure : ONAIR 없음. 생방송 상품 시간 아님.");
				assertTrue(true);
			}
		} catch (Exception e) {
			e.printStackTrace();
