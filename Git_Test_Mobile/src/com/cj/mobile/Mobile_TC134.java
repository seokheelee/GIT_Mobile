 ...
 * Subject : CJ Mall 
 * Name : TC_134
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 클릭 시 마이존 페이지로 이동
 * Assertion : 브라우저 Title "마이존" 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */

public class Mobile_TC134 {
	...
	@Test
	public void MyzoneLogin() throws Exception {
		...
		try {
		    ...
		    // 브라우저 Title "마이존" 체크
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/span"), "마이존");
		    if(isExist2 && driver.getTitle().equals("마이존")){
		    	System.out.println("[TC_134] success");
				assertTrue(true);
				return;
		    } else {
		    	System.out.println("[TC_134] failure : 브라우저 Title '마이존' 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
