 ...
 * Subject : CJ Mall 
 * Name : TC_029
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "디지털/여행" 카테고리 노출
 * Assertion : "디지털/여행" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC029 {
	...
	@Test
	public void digitalTourView() throws Exception {
		...
		try {
			...
			// 인테리어/리빙 클릭 (카테고리바 이동 목적)
		  	...
		  	Thread.sleep(3000);		  	
		  	// "디지털/여행" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[8]/a/span"),"디지털/여행");
		  	if(isExist){
		  		if("디지털/여행".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[8]/a/span")).getText())){
		  			System.out.println("[TC_029] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_029] failure : '디지털/여행' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_029] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
