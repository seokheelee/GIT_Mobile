 ...
 * Subject : CJ Mall 
 * Name : TC_016
 * Scenario : 전시 > 오클락 딜 > 지금 방송중인 다른상품 > 가격 노출
...
public class Mobile_TC016 {
	...
	@Test
	public void otherProductPriceView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	if(isExist)
		  	{
		  		if(!"".equals(driver.findElement(By.xpath(".//*[@id='prd_flickwrap']/div/ul[1]/li[1]/a/div[2]/span[2]")).getText())){
		  			System.out.println("[TC_016] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_016] failure : 상품가격 null...");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_016] failure : '지금 방송중인 다른상품' 방송시간대 아님. 여기서 종료 ");
	  			assertTrue(true);
	  		   	return;
		  	}
		} catch (Exception e) {
