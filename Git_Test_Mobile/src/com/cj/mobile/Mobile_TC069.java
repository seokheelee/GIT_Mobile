 ...
 * Subject : CJ Mall 
 * Name : TC_069
 * Scenario : 전시 > 하단 '최근본상품' 버튼
 * Assertion :  "최근본상품" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC069 {
	...
	@Test
	public void LastView() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[6]/a")));
		  	System.out.println("하단 '최근 본 상품' 버튼 기다림");
			
			 // 하단 '최근 본 상품' 버튼 클릭
			driver.findElement(By.xpath(".//*[@id='o1h_footer']/nav/ul/li[6]/a")).click();
		  	System.out.println("하단 '최근 본 상품' 버튼 클릭");
			
			// "최근 본 상품" Text 기다림.
		    wait = new WebDriverWait(driver, waitTime);
		  	wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/span")));
		  	System.out.println("'최근 본 상품' text 기다림");
		  	
		  	// "최근 본 상품" Text 체크
			if ("최근 본 상품".equals(driver.findElement(By.xpath(".//*[@id='o1h_header_wrap']/div/span")).getText())) {
				System.out.println("[TC_069] success");
				assertTrue(true);
				return;
			} else {
		    	System.out.println("[TC_069] failure : '최근 본 상품' Text 불일치");
				assertTrue(false);
			}
