 ...
 * Subject : CJ Mall 
 * Name : TC_176
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑찜 > Check box Check 확인
 * Assertion : Check Box 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */


public class Mobile_TC176 {
	...
	@Test
	public void MyzoneShoppingzzimCheckBox() throws Exception {
		...
		try {
			...
			// 쇼핑찜 버튼 클릭
	  		...
	  		// CheckBox 기다림
	  		wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='del_idx0']")));
		  	System.out.println("CheckBox 기다림");
			
			// CheckBox 클릭
			element = driver.findElement(By.xpath(".//*[@id='del_idx0']"));
	  		element.click();
		  	System.out.println("CheckBox 클릭");			
	  		
			// Check Box 체크
			if ("".equals(driver.findElement(By.xpath(".//*[@id='del_idx0']")).getText())) {
				System.out.println("[TC_176] success");
				assertTrue(true);
				return;
				
			} else {
		    	System.out.println("[TC_176] failure : 'Check Box' 불일치");
				assertTrue(false);
			}
