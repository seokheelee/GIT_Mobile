 ...
 * Subject : CJ Mall 
 * Name : TC_179
 * Scenario : 전시 > 상단메뉴 > 로그인 > 마이존 > 쇼핑찜 > 전체선택
 * Assertion : 전체선택->전체해제 로 변경 후 전체해제 Text 체크
...
public class Mobile_TC179 {
	...
	@Test
	public void MyzoneShoppingzzimAllClear() throws Exception {
		...
		try {
		  	...
		  	if(driver.findElement(By.xpath(".//*[@id='ct']/form/div[2]/div/label")).getText().indexOf("전체선택") >=0) 
			{
		    	...
		    	System.out.println("[TC_179] failure : 전체해제 불일치");
				assertTrue(false);
			}
