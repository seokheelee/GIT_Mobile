 ...
 * Subject : CJ Mall 
 * Name : TC_003
 * Scenario : 전시 > 상단메뉴 > 타이틀바 > 검색 창 및 검색 버튼 노출 
 * Assertion : XPath 값 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 */

public class Mobile_TC003 {
	...
	@Test
	public void cjmallSearchView() throws Exception {
		...
		try {
			...
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/dl/dd/div/fieldset/input")));
			System.out.println("검색창 기다림");

			// 검색창 클릭
			element = driver.findElement(By.xpath(".//*[@id='o1h_header_wrap']/dl/dd/div/fieldset/input"));
	  		element.click();
	  		System.out.println("검색창 클릭");
	  		
	  		// 검색버튼 기다림
			wait = new WebDriverWait(driver, waitTime);
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[@id='o1h_header_wrap']/div/div/div/div/button[2]")));
			System.out.println("검색버튼 기다림");
			
			boolean isExist2 = existElement(driver, By.xpath(".//*[@id='o1h_header_wrap']/div/div/div/div/button[2]"), "");
			if(isExist2){
				System.out.println("[TC_003] success");
				assertTrue(true);
				return;
			} else {
				System.out.println("[TC_003] failure : 검색버튼 XPath 불일치");
				assertTrue(false);
			}
		} catch (Exception e) {
			e.printStackTrace();
