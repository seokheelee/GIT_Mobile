 ...
 * Subject : CJ Mall 
 * Name : TC_028
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "인테리어/리빙" 카테고리 노출
 * Assertion : "인테리어/리빙" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC028 {
	...
	@Test
	public void interiorLivingView() throws Exception {
		...
		try {
			...
			// 유아동/도서 클릭 (카테고리바 이동 목적)
		  	...
		  	// "인테리어/리빙" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[7]/a/span"),"인테리어/리빙");
		  	if(isExist){
		  		if("인테리어/리빙".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[7]/a/span")).getText())){
		  			System.out.println("[TC_028] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_028] failure : '인테리어/리빙' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_028] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
