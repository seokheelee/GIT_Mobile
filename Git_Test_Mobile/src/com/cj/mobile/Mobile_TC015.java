 ...
 * Subject : CJ Mall 
 * Name : TC_015
 * Scenario : 전시 > 오클락 딜 > 지금 방송중인 다른상품 > 상품 타이틀 노출
...
public class Mobile_TC015 {
	...
	@Test
	public void otherProductNameView() throws Exception {
		...
		try {
			...
			if(isExist4){
		  	...
		  	if(isExist)
		  	{
		  		if(!"".equals(driver.findElement(By.xpath(".//*[@id='prd_flickwrap']/div/ul/li/a/div[2]/span[1]")).getText())){
		  			System.out.println("[TC_015] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_015] failure : 상품명 null...");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_015] failure : '지금 방송중인 다른상품' 방송시간대 아님. 여기서 종료 ");
	  			assertTrue(true);
	  		   	return;
		  	}
		} catch (Exception e) {
