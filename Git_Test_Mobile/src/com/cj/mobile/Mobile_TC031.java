 ...
 * Subject : CJ Mall 
 * Name : TC_031
 * Scenario : 전시 > 오클락 딜 > 딜 카테고리 > "백화점" 카테고리 노출
 * Assertion : "백화점" Text 체크
 * update : ScreenRecorder 기능 제거 (2016-06-16)
 * update : Click 이벤트 변경 (2016-11-28)
 * 
 */

public class Mobile_TC031 {
	...
	public void tvShoppingView() throws Exception {
		...
		try {
			...
			// TV쇼핑 클릭 (카테고리바 이동 목적)
		  	driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[9]/a/span")).click(); 
		  	System.out.println("TV쇼핑 클릭");
			
		  	// "백화점" Text 체크
		  	isExist = existElement(driver,By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[10]/a/span"),"백화점");
		  	if(isExist){
		  		if("백화점".equals(driver.findElement(By.xpath(".//*[@id='ct']/div/div[8]/div[1]/div/div/ul/li[10]/a/span")).getText())){
		  			System.out.println("[TC_031] success");
					assertTrue(true);
					return;
		  		} else {
		  			System.out.println("[TC_031] failure : '백화점' Text 불일치");
		  			assertTrue(false);
		  		   	return;
		  		}
		  	} else {
		  		System.out.println("[TC_031] failure : Time out...");
	  			assertTrue(false);
	  		   	return;
		  	}
		} catch (Exception e) {
